 ...
 *  1. FC_ExportDataReady is called by indivdual charts and ASCII data with
 ...
 *     to accept data. When it attains readyness, it checks whether there is
 ...
 *  well as temporally. (2) Light prototype definition... alloqing for lesser
...
{
    alert('FusionChartsExport: object::FusionChart was not found.'
...
{
    // initializeComponent constructor routines (that has been declared
    // separately)
...
_FCEO.defaultParameters = {
    ...
    componentAttributes: {
        width: '120',  // width of the SWF componet (required)
        ...
        strictTrigger: '1'  // whether the handlerId is overrideable for charts
    },
    exportAttributes: { },
    sourceCharts: [],
...
{
    // call the batchExport() function with the sourcecharts as parameter
...
/**
 *  Ovverride the toString() to return valid component name when alerted.
...
_FCEO.relayStream = function(streamObj)
{
	...
	// bubble event ... this is done as timeout, so that this script doesnt get 
	// hung when the external script is heavy
	setTimeout(function() { 
		try {
			//FC_ExportReady(streamObj.meta.DOMId); 
			} catch(err) {} } , 0);
    ...
    if( _FCEO.probeComponent(sHandlerId) ) {
        _FCEO.L.get(sHandlerId).pushData([streamObj]);
        return true;
    }
...
_FCEO.flushRack = function(sHandlerId)
{
    ...
    _FCEO.L.get(sHandlerId).pushData(_FCEO.streamRack[sHandlerId]);
...
/**
 *  Determines whether a particular HTML DOM element is a FusionCHart or not.
...
_FCEO.probeChart = function(obj)
{
    ...
    return ( _FCEO.L.isString(sig) &&  sig.indexOf('FusionCharts/') == 0 );
};
...
_FCEO.initializeComponent = function(obj, args)
{
    ...
    // initializeComponent all intrnal properties from the defaulSettings definition
...
_FCEO.updateComponent = function(obj) {
    ...
    // set initial and other related values by synchronizing component atributes
...
_FCEO.L = {
     ...
     *  @param nam string: the node tag name that is to be looked up withn DOM
     ...
     *  @param id string: the id that is to be looked up withn DOM
     ...
     *  @param obj object: The object whose tyoe is to be verified
     *  @return boolean
     */
     ...
     *  @param num integer: Error number (follow a convenstion)
     */
